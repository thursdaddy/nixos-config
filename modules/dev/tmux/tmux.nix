_: {
  ...
  flake.modules.homeManager.dev =
    ...
    let
      cfg = osConfig.mine.home-manager.tmux;
    ...
    in
    {
      home.packages = with pkgs; [
        ...
        (lib.mkIf (cfg.sessionizer.enable && user.shell.package == pkgs.zsh) tmuxs_zsh)
        tmux
        fzf
      ];

      home.file = lib.mkIf (cfg.sessionizer.enable && user.shell.package == pkgs.zsh) {
      ...
      programs.zsh = lib.mkIf (cfg.sessionizer.enable && user.shell.package == pkgs.zsh) {
        initContent = ''
