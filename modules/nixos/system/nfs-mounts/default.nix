{ lib, config, ... }:
let
  inherit (lib) mkEnableOption mapAttrs mapAttrsToList mkIf mkMerge mkOption types;
    ...
    mounts = mkOption {
      ...
      type = types.attrsOf (lib.types.submodule {
        ...
        options = {
          ...
          options = mkOpt (types.listOf types.str) [ "auto" "rw" "defaults" "_netdev" ] "Default mount options";
        };
      });
    };
  };
